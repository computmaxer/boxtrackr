...
def check_message_for_tracking_number(message):
    ...
    _REGEX = {
        'ups': re.compile(r"(1Z?[0-9A-Z]{3}?[0-9A-Z]{3}?[0-9A-Z]{2}?[0-9A-Z]{4}?[0-9A-Z]{3}?[0-9A-Z]|[\dT]\d\d\d?\d\d\d\d?\d\d\d)"),
        'usps': re.compile(r'(E\D{1}\d{9}\D{2}$|9\d{15,21})'),
    }

    for type, regex in _REGEX.items():
